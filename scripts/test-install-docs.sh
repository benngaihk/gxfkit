...
The current public Bioconda package is `0.0.1`.
It is useful for basic installs, but it predates the no-overwrite guard; use
the release status before treating a Bioconda package as strict-audit production
evidence.
MD
...
make_fixture "$fixture"
perl -0pi -e 's/The current public Bioconda package is `0\.0\.1`\.//' \
  "$fixture/README.md"
expect_fail \
  missing-bioconda-version-boundary \
  'README.md must mention: The current public Bioconda package is `0.0.1`' \
  env GXFKIT_ROOT="$fixture" "$PY" "$ROOT/scripts/check-install-docs.py"

fixture="$tmp/missing-zh-bioconda-boundary"
make_fixture "$fixture"
perl -0pi -e 's/当前公开的 Bioconda 包是 `0\.0\.1`，可用于基础安装验证，但早于“拒绝覆盖输出文件”保护；\n判断某个 Bioconda 包能否作为严格生产证据前，请以发布状态文档为准。//' \
  "$fixture/README.zh-CN.md"
expect_fail \
  missing-zh-bioconda-boundary \
  'README.zh-CN.md must mention: 当前公开的 Bioconda 包是 `0.0.1`' \
  env GXFKIT_ROOT="$fixture" "$PY" "$ROOT/scripts/check-install-docs.py"
